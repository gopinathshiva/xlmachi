        ...
        <div class="row">
            <?php if( $_offer_type == 'deal' ): ?>
                <div class="col-sm-8">
                    ...
                    if($num_comments==0){
                        $xl_comment_title = 'Add a Comment';
                    }else if($num_comments==1){
                        $xl_comment_title = $num_comments.' Comment';
                    }else{
                        $xl_comment_title = $num_comments.' Comments';
                    }
                    ?>
                    <div class="white-block xl-comment-show-block">
                        <div class="white-block-title">
                            <i class="fa fa-ticket"></i>
                            <h2><?php echo $xl_comment_title.' (+)' ?></h2>
                        </div>
                    </div>
                    <div class='xl-comment-block'>
                ...
                <div class="hidden" vocab="http://schema.org/" typeof="Product">
                  <span property="name"><?php the_title() ?></span>
                    ...
                    if( has_post_thumbnail() ){
                        ...
                        echo '<img propertyu="image" src="'.esc_url( $image_data[0] ).'" alt="" /> ';
                    }
                    ?>  
                  
                  <span property="description"><?php echo get_the_excerpt(); ?></span>

                  <span property="aggregateRating" typeof="AggregateRating">
                    <span property="ratingValue"><?php echo number_format( $result['sum'], 2 ); ?></span> <?php _e( ' stars, based on ', 'couponxl' ) ?> <span property="reviewCount"><?php echo $result['count']; ?>
                      </span> <?php _e( ' rates', 'couopnxl' ) ?>
                  </span>

                  <span property="offers" typeof="Offer">
                    <?php _e( 'Regular price: ', 'couponxl' ) ?> <?php echo couponxl_format_price_number( $deal_price ) ?>
                    <meta property="priceCurrency" content="<?php echo esc_attr( couponxl_get_option( 'main_unit_abbr' ) ) ?>" />
                    <?php echo couponxl_get_option( 'unit' ) ?><span property="price"><?php echo $deal_sale_price ?></span>
                    <?php
                    if( !empty( $offer_expire ) && $offer_expire != '99999999999' ){
                        ?>
                        (<?php _e( 'Sale ends', 'couponxl' ) ?> <time property="priceValidUntil" datetime="<?php echo date_i18n( 'Y-m-d', $offer_expire ) ?>"> <?php echo date_i18n( 'F j, y', $offer_expire ) ?>!</time>)                        
                        <?php
                    }
                    ?>
                    <?php echo _e( 'Available from: ', 'couponxl' ) ?> <span property="seller"><?php echo get_the_title( $offer_store ); ?></span>
                  </span>
                </div>                
            <?php else: ?>
                <div class="col-sm-8">
