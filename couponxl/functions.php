...
function couponxl_store_url(){
	...
	if ( is_tax() ) {
		...
		if( !empty( $couponxl_slugs[$taxonomy] ) ){
			$permalink = couponxl_get_permalink_by_tpl( 'page-tpl_search_page' );
			if( $taxonomy == 'letter' ){
				$permalink = couponxl_get_permalink_by_tpl( 'page-tpl_all_stores' );
			}
			$term = get_query_var( 'term' );
			$url = couponxl_append_query_string( $permalink, array( $taxonomy => $term ) );
			wp_redirect( $url );
		}
	}
}
		...
		if( empty( $first_name ) || empty( $phone_number ) ){
			$return = '
				<form class="payu-form payu-additional">
					'.( empty( $first_name ) ? '<label for="payu_name">'.__( 'First Name', 'couponxl' ).'</label><input type="text" id="payu_name" name="payu_name" class="form-control">' : '' ).'
					'.( empty( $phone_number ) ? '<label for="payu_phone">'.__( 'Phone Number', 'couponxl' ).'</label><input type="text" id="payu_phone" name="payu_phone" class="form-control">' : '' ).'
					<input type="hidden" name="offer_id" value="'.esc_attr( $offer_id ).'">
					<input type="hidden" name="return_url" value="'.esc_url( $return_url ).'">
					<input type="hidden" name="status_url" value="'.esc_url( $return_url ).'">
					<input type="hidden" name="action" value="payu_additional" />
					<a href="javascript:;" class="payu-additional-info">'.__( 'Process', 'couponxl' ).'</a>
				</form>
			';
		}
		else{
			...
			$return = '
				<form class="payu-form '.( !empty( $_POST['offer_id'] ) ? 'payu-submit' : 'payu-submit-click' ).'" action="https://'.( couponxl_get_option( 'payumode' ) ).'.payu.in/_payment" method="post">
...
function couponxl_generate_paypal_offer_link( $offer_id = '' ){
	...
	if( !empty( $deal_link ) ){
		echo '<a href="'.esc_url( add_query_arg( array( 'rd' => get_the_ID() ), get_permalink() ) ).'" class="btn" target="_blank">'.__( 'BUY NOW', 'couponxl' ).'</a>';
	}
...
function couponxl_get_voucher_code( $offer, $transaction_id = '', $voucher_buyer_id = '' ){
	...
	if( empty( $voucher_exists ) ){
		...
		return __( 'Thank you for your purchase. Your voucher code is: <strong>', 'couponxl' ).$voucher_code.'<strong> <a href="javascript:;" class="print-voucher pull-right">'.__( 'Print', 'couponxl' ).'</a> </strong>
...
require_once( locate_template( 'includes/update.php' ) );
